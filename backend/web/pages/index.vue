  ...
  <div v-if="!systemConfig?.maintenance_mode" class="min-h-screen bg-gray-50 dark:bg-slate-900 text-gray-800 dark:text-slate-100 flex flex-col">
    ...
    <main class="flex-1 p-3 sm:p-5">
      ...
      <header class="header-container bg-slate-800 dark:bg-slate-800 text-white dark:text-slate-100 rounded-lg shadow-lg p-4 sm:p-8 mb-4 sm:mb-8 text-center relative">
        <h1 class="mr-14 mb-4 flex min-w-0 items-center justify-start gap-2 text-xl font-bold sm:mr-0 sm:justify-center sm:gap-3 sm:text-3xl">
          <img 
          ...
          <img 
            ...
            class="h-8 w-auto object-contain"
          />
          <a href="/" class="min-w-0 truncate text-white no-underline hover:text-gray-200 dark:hover:text-gray-300">
      ...
      <section aria-label="搜索功能" class="w-full max-w-3xl mx-auto mb-4 sm:mb-8 px-2 sm:px-0">
        ...
        <!-- 平台类型筛选 -->
        <nav v-if="!hasSearchQuery" aria-label="平台筛选" class="mt-3 flex flex-wrap gap-2" id="platformFilters">
          <a 
      ...
      <ClientOnly v-if="hasSearchQuery">
        <MelostSearchResults :query="String(route.query.search)" />
      </ClientOnly>

      <!-- 无搜索词时展示系统内最新资源 -->
      <section v-else aria-label="资源列表" class="overflow-x-auto bg-white dark:bg-slate-800 rounded-lg shadow-lg shadow-slate-900/10 dark:shadow-slate-900/50">
