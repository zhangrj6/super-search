...
var melostTransferGroup singleflight.Group
...
func GetResourceLink(c *gin.Context) {
	...
	if resource.Source == "melost" || resource.Source == "quanpan" {
		...
		value, transferErr, _ := melostTransferGroup.Do(strconv.FormatUint(uint64(resource.ID), 10), func() (interface{}, error) {
			freshResource, findErr := repoManager.ResourceRepository.FindByID(resource.ID)
			if findErr != nil {
				return nil, fmt.Errorf("重新读取资源失败: %w", findErr)
			}
			if freshResource.SaveURL != "" {
				return freshResource.SaveURL, nil
			}

			transferResult := services.PerformAutoTransfer(
		...
		saveURL, ok := value.(string)
		if !ok || strings.TrimSpace(saveURL) == "" {
			ErrorResponse(c, "转存失败：网盘没有返回新的分享链接", http.StatusBadGateway)
			return
		}
		SuccessResponse(c, gin.H{
			"url":         saveURL,
