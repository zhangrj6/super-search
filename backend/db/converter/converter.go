...
func ToResourceResponseFromMeilisearch(doc interface{}) dto.ResourceResponse {
	...
	if sourceField := docValue.FieldByName("Source"); sourceField.IsValid() {
		response.Source = sourceField.String()
	}
	if externalIDField := docValue.FieldByName("ExternalID"); externalIDField.IsValid() {
		response.ExternalID = externalIDField.String()
	}
